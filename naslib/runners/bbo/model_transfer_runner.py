...
search_space = get_search_space(config.test_search_space, config.test_dataset)
search_space.instantiate_model = False
search_space.sample_without_replacement = True
zc_api = get_zc_benchmark_api(config.test_search_space, config.test_dataset)
search_space.labeled_archs = [eval(arch) for arch in zc_api.keys()]
print(search_space, config.test_dataset)
evaluator.adapt_search_space(search_space, config.test_dataset, dataset_api, config)
evaluator.zc_api = zc_api
evaluator.test(ensemble, zc_predictors, train_loader)

...
hyperparams = {}
if config.param == "hidden_layer_size":
    if int(config.value) < 1000:
        hyperparams["hidden_layer_sizes"] = (int(config.value),)
    else:
        sizes = []
        size_encoding = int(config.value)
        while size_encoding > 0:
            sizes.append(size_encoding % 1000)
            size_encoding = size_encoding // 1000
        hyperparams["hidden_layer_sizes"] = tuple(sizes)

if config.param == 'activation':
    hyperparams['activation'] = config.value

if config.param == 'learning_rate_init':
    hyperparams['learning_rate_init'] = float(config.value)

config.save = config.save.replace(config.dataset, os.path.join(config.dataset, config.param, config.value))

...
do
    ...
    do
        bash scripts/bash_scripts/xgb_correlation/create_configs.sh $experiment $searchspace $dataset $start_seed $k "synflow plain l2_norm flops snip grad_norm nwot zen fisher jacov epe_nas params grasp" $predictor # "synflow flops params"
        for size in "${train_sizes[@]}"
        do
            bash ./scripts/cluster/xgb_correlation/run.sh $searchspace $dataset $size $start_seed $experiment $k $n_seeds
        done
    done
done
done

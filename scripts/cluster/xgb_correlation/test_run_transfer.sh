...
train_sizes=(5 8 14 24 42 71 121 1000)
...
do
    ...
    do
        bash scripts/bash_scripts/xgb_correlation/create_configs.sh $experiment $searchspace $dataset $start_seed $k "synflow plain l2_norm flops snip grad_norm nwot zen fisher jacov epe_nas params grasp" $predictor $test_searchpace $test_dataset # "synflow flops params"
        for size in "${train_sizes[@]}"
        do
            bash ./scripts/cluster/xgb_correlation/run_transfer.sh $searchspace $dataset $size $start_seed $experiment $k $n_seeds
        done
    done
done
done
